...
#else
        // 2 OUT pins for H & VSYNC, or 1 for CSYNC and 1 possibly sideset pin following them
        pin_count = 1 + ((video_mode.default_timing->h_sync_polarity & (CSYNC_EXTEND + CSYNC_SUPPRESS)) ? 0 : 1);
#endif
...
bool scanvideo_setup_with_timing(const scanvideo_mode_t *mode, const scanvideo_timing_t *timing) {
    ...
    uint pin_mask = 1u << PICO_SCANVIDEO_SYNC_PIN_BASE;
    bi_decl_if_func_used(bi_1pin_with_name(PICO_SCANVIDEO_SYNC_PIN_BASE + 0, "HSync"));

    if (!(mode->default_timing->h_sync_polarity & (CSYNC_EXTEND + CSYNC_SUPPRESS))) {
        pin_mask |= 2u << PICO_SCANVIDEO_SYNC_PIN_BASE;
        bi_decl_if_func_used(bi_1pin_with_name(PICO_SCANVIDEO_SYNC_PIN_BASE + 1, "VSync"));
    }
#if PICO_SCANVIDEO_ENABLE_DEN_PIN
